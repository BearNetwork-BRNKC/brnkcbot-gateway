...
import { UniswapLPHelper } from './smbswap.lp.helper';
import { AddPosReturn } from './smbswap.lp.interfaces';
...
export class UniswapLP extends UniswapLPHelper implements UniswapLPish {
  ...
  async getPosition(tokenId: number): Promise<PositionInfo> {
    ...
    const positionInst = new smbV3.Position({
      pool: new smbV3.Pool(
        token0,
        token1,
        poolData.fee,
    ...
    return {
      ...
      fee: smbV3.FeeAmount[position.fee],
