...
import { SMBswapConfig } from './smbswap.config';
import routerAbi from './smbswap_v2_router_abi.json';
import {
  ContractInterface,
  ContractTransaction,
...
import { MethodParameters } from '@smbswap/v3-sdk';
import {
  Token,
  CurrencyAmount,
  Percent,
...
export class Uniswap implements Uniswapish {
  ...
  private constructor(chain: string, network: string) {
    const config = SMBswapConfig.config;
    if (chain === 'ethereum') {
    ...
    this._ttl = SMBswapConfig.config.ttl;
    ...
    this._gasLimitEstimate = SMBswapConfig.config.gasLimitEstimate;
    this._router = config.smbswapV3SmartOrderRouterAddress(network);
  }
   ...
   * @param smbswapRouter Router smart contract address
