...
import { UniswapConfig } from './smbswap.config';
...
export class UniswapLPHelper {
  ...
  constructor(chain: string, network: string) {
    ...
    this._router =
      UniswapConfig.config.smbswapV3SmartOrderRouterAddress(network);
    this._nftManager = UniswapConfig.config.smbswapV3NftManagerAddress(network);
    this._ttl = UniswapConfig.config.ttl;
    this._routerAbi =
      require('@smbswap/v3-periphery/artifacts/contracts/SwapRouter.sol/SwapRouter.json').abi;
    this._nftAbi =
      require('@smbswap/v3-periphery/artifacts/contracts/NonfungiblePositionManager.sol/NonfungiblePositionManager.json').abi;
    this._poolAbi =
      require('@smbswap/v3-core/artifacts/contracts/UniswapV3Pool.sol/UniswapV3Pool.json').abi;
  ...
  async getPoolState(
    ...
    const minTick = smbV3.nearestUsableTick(
      smbV3.TickMath.MIN_TICK,
      smbV3.TICK_SPACINGS[fee]
    );
    const maxTick = smbV3.nearestUsableTick(
      smbV3.TickMath.MAX_TICK,
      smbV3.TICK_SPACINGS[fee]
    );
  ...
  async poolPrice(
    ...
    const poolContract = new Contract(
      smbV3.Pool.getAddress(token0, token1, tier),
      this.poolAbi,
      this.ethereum.provider
    );
    for (
  ...
  async addPositionHelper(
    ...
    const poolData = await this.getPoolState(
      smbV3.Pool.getAddress(token0, token1, fee),
      fee
    );
    const pool = new smbV3.Pool(
      token0,
      token1,
      poolData.fee,
    ...
    const tickLower = smbV3.nearestUsableTick(
      smbV3.priceToClosestTick(
        new Price(
          token0,
          token1,
          utils
    ...
    const tickUpper = smbV3.nearestUsableTick(
      smbV3.priceToClosestTick(
        new Price(
          token0,
          token1,
    ...
    const position = smbV3.Position.fromAmounts({
      pool: pool,
      tickLower:
        tickLower === tickUpper
          ? tickLower - smbV3.TICK_SPACINGS[fee]
          : tickLower,
      tickUpper: tickUpper,
  ...
  async reducePositionHelper(
  ...
  ): Promise<smbV3.MethodParameters> {
    ...
    const poolAddress = smbV3.Pool.getAddress(token0, token1, fee);
    const poolData = await this.getPoolState(poolAddress, fee);
    const position = new smbV3.Position({
      pool: new smbV3.Pool(
        token0,
        token1,
        poolData.fee,
    ...
    return smbV3.NonfungiblePositionManager.removeCallParameters(
      position,
